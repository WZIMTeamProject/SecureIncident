...
export function Background({ children, className = "" }: PageBgProps) {
    return (
        <div className={`min-h-screen flex flex-col bg-[var(--color-si-page-bg)] transition-colors duration-300 ${className}`}>
            {/* Background shield icons */}
            <div className="pointer-events-none select-none fixed inset-0 z-0 overflow-hidden">
                <Shield className="absolute left-6 top-40 w-92 h-92 text-[var(--color-si-shield)] opacity-10" />
                <Shield className="absolute left-1/2 top-20 w-52 h-52 text-[var(--color-si-shield)] opacity-10 mt-5" />
                <Shield className="absolute right-6 bottom-20 w-72 h-72 text-[var(--color-si-shield)] opacity-15" />
            </div>
